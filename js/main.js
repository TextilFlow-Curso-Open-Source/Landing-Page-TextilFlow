...
document.addEventListener('DOMContentLoaded', function() {
  ...
  if (howItWorksBtn && contactSection) {
...
function openModal(plan, price) {
  ...
  // Actualizar el tipo de plan en el formulario
  document.getElementById('plan-type').value = plan;

  // Obtener el select de opciones de pago
  ...
  // Agregar opciones según el tipo de plan
  if (plan === 'Plan Básico' || plan === 'Basic Plan') {
    paymentSelect.innerHTML = `
      <option value="mensual">Mensual - $9.99</option>
      <option value="anual">Anual - $99.99</option>
    `;
  } else if (plan === 'Plan Corporativo' || plan === 'Corporate Plan') {
    paymentSelect.innerHTML = `
      <option value="mensual">Mensual - $49.99</option>
      <option value="anual">Anual - $499.99</option>
    `;
  }
  // *** HASTA AQUÍ ***

  const form = document.querySelector('#payment-modal .checkout-form');
  const payButton = form.querySelector('.submit-button');
